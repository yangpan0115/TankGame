...
public class MyPanel extends JPanel implements Runnable,KeyListener {
    ...
    public MyPanel(){
        ...
        for(int i=0;i<ensize;i++){
            //创建敌人的坦克对象
            EnemyTank et1 = new EnemyTank(i*200,50,Direction.D);
            //启动敌方坦克
            Thread t1 = new Thread(et1);
            t1.start();
            //给敌人添加子弹
            Bullet bullet1 = new Bullet(et1.x,et1.y,Direction.D);
            //加入给敌方坦克
            et1.bullets.add(bullet1);
            //发射子弹
            Thread tt1 = new Thread(bullet1);
            tt1.start();
            //加入坦克群
            ets.add(et1);
        }
    ...
    protected void paintComponent(Graphics g){
        ...
        //背景填充
        screenImage = this.createImage(PanelWidth,PanelHeight);
        Graphics gps = screenImage.getGraphics();
        Color c1 = gps.getColor();
        gps.setColor(Color.GRAY);
        gps.fillRect(0, 0, PanelWidth, PanelHeight);
        gps.setColor(c1);
        ...
        for (int i = 0; i < ets.size(); i++) {
            ...
            if (et.isLive) {
                ...
                for (int j = 0; j < et.bullets.size(); j++) {
                    ...
                    if (enemyBullet.isLive&&et.time>=5) {
