    ...
    protected void paintComponent(Graphics g){
        ...
        for(int i=0;i<myTanks.size();i++){
            ...
            if(myTank.isLive){
                ...
                //从ss中取出每一颗子弹，并画出
                for(int j=0;j<myTank.ss.size();j++){
                    Shot myShot = myTank.ss.get(j);
                    //画出子弹
                ...
                //从mm中取出每颗导弹，并画出
                for(int j=0;j<myTank.mm.size();j++){
                    Missile missile = myTank.mm.get(j);
                    //画出导弹
        ...
        if (ets.size() == 0 && home.isLive && myTanks.size() != 0) {
            //gps.clearRect(0,0,PanelWidth,PanelHeight);
            Font f = g.getFont();
